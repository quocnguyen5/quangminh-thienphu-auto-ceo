...
PROXY_LIST = {
    ...
    "3": "100009773418904.ldproxy.com:14107",
...
search_keys = [
    ...
    # ("Thiên Phú", f"đáo hạn thẻ tín dụng đà nẵng thiên phú"),
]
...
def click_direction(driver):
    ...
    for element in elements:
        ...
        if "Đường đi" in website_string.get_attribute(
            "innerHTML"
        ) or "Chỉ đường" in website_string.get_attribute("innerHTML"):
            element.click()
            time.sleep(random.randint(3, 7))
            ...
            for item in names:
                if key in item.get_attribute("innerHTML"):
                    ...
                    if CLICK_MAPS is True:
                        click_direction(driver)

                    if CLICK_CALL is True and random.randint(0, 1) == 1:
                        click_call(driver)
                    break
            print(f"Done ..... {i}")
            driver.quit()
